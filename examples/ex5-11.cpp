...
Person::Person(const Person& person) {
    ...
    int len = strlen(person.name); // name의 문자개수 저장
    this -> name = new char[len+1]; //name에 len 만큼 동적할당
    strcpy(this -> name, person.name); // 복사
    cout << "복사 생성자 실행. 원본객체의 이름" << this-> name << endl;
}
Person::~Person(){ // 소멸자
    if (name) {
        delete [] name;
